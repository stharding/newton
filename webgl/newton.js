...
class Newton {
  constructor() {
    this.gl,
      ...
      this.oy = cy,
      this.iterations = 100,
      this.minX = cx - (x_factor * scale),
      this.maxX = cx + (x_factor * scale),
      this.minY = cy - (y_factor * scale);
  }
  ...
  init() {
    ...
    if (first) {
      ...
      program = initShaders(gl, "newton-vertex.glsl", "newton-fragment.glsl");
...
class Polynomial {
  ...
  derivative() {
    ...
    for (let i = 0; i < this.coefficients.length - 1; ++i) {
      derived_coeffs.push(this.coefficients[i] * exponent);
      exponent--;
    }
    return new Polynomial(derived_coeffs)
  }
}
